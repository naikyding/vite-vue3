...
const getOneCityTourism = (filterData) => store.dispatch('get_tourism', filterData)
// getAllTourism()
      ...
      <section class="mt-5">
        ...
        <div class="search-select shadow-xl border rounded-xl mt-8 px-4 py-5">
          ...
          <div class="select-area w-full relative">
            <select v-model="state.form.city" class="w-full appearance-none bg-input-bg border text-gray-700 py-3 px-4 pr-8 rounded-xl leading-tight focus:outline-none focus:border-primary">
          ...
          <!-- 全部 類別 -->
          <label class="block">
            <input v-model="state.form.class" value="all" class="mr-2" name="class" type="radio">
            <span class="text-primary3">全部</span>
          </label>

          <label class="block" v-for="item in state.class" :key="item">
            <input v-model="state.form.class" :value="Object.values(item)[0]" class="mr-2" name="class" type="radio" >
        ...
        <div class="result-items col-span-3 mt-8">
          <figure class="bg-white border shadow-lg rounded-2xl flex overflow-hidden mb-5 md:mb-10 h-32 md:h-60" v-for="item in stateTourismData" :key="item.ID">
            ...
            <figcaption class="px-2 md:px-4 py-3 md:py-5 flex-1 flex flex-col">
              <!-- TITLE && TAGS -->
              <div class="flex-grow md:flex-grow-0 figcaption-header flex flex-col justify-between mb-2">
                <h2 class="block text-base md:text-xl text-secondary">
                  {{ item.Name }}
                </h2>
                <!-- TAG (CLASS) -->
                <div class="tag-area hidden md:flex">
                  <span v-if="item.Class1" class="text-center text-xs tag bg-primary text-white px-3 py-1 md:flex items-center rounded-full">
                    {{ item.Class1}}
                  </span>
                  <span v-if="item.Class2" class="text-center text-xs mx-2 tag bg-primary text-white px-3 py-1 md:flex items-center rounded-full">
                    {{ item.Class2}}
                  </span>
                  <span v-if="item.Class3" class="text-center text-xs tag bg-primary text-white px-3 py-1 md:flex items-center rounded-full">
                    {{ item.Class3}}
                  </span>
                </div>
              ...
              <div class="hidden md:block description text-gray-500 flex-grow">
                {{ item.DescriptionDetail.length > 70 ? item.DescriptionDetail.slice(0, 70) + '...' : item.DescriptionDetail }}
              </div>
              ...
              <div class="flex">
                ...
                <div class="location flex items-center">
                  <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-priamry2 mr-1" viewBox="0 0 20 20" fill="currentColor">
                ...
                <div class="hidden md:flex open-time ml-4 items-center">
                  ...
                  <span class="text-gray-500">{{ item.OpenTime?.length > 10 ? item.OpenTime.slice(0, 10) + '...' : item.OpenTime }}</span>
                </div>
              </div>
            </figcaption>
          </figure>
